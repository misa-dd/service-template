    ...
    stage('Deploy to prod') {
      steps {
        script {
          ...
          deployService(params['GITHUB_REPOSITORY'], params['SHA'], 'prod', common.getServiceName(), rollback: true)
        }
      }
      post {
        success {
          ...
          sendSlackMessage 'eng-deploy-manifest', "Successful rollback of ${common.getServiceName()} to ${tag}: <${BUILD_URL}|${env.JOB_NAME} [${env.BUILD_NUMBER}]>"
        }
        failure {
          script {
            tag = getImmutableReleaseSemverTag(params['SHA'])
          }
          sendSlackMessage 'eng-deploy-manifest', "Rollback failed for ${common.getServiceName()} to ${tag}: <${BUILD_URL}|${env.JOB_NAME} [${env.BUILD_NUMBER}]>"
        }
      }
    }
